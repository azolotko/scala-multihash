...
object MultiHash {
  ...
  sealed abstract class HashType(val name: String, val index: Byte, val length: Byte) extends Serializable
  ...
  /** Return a MultiHash of `contents` using SHA-256. This is the IPFS default hash algorithm.
    *
    * @param contents
    *   an array of bytes to hash
    * @return
    *   a MultiHash describing the hashed contents
    */
  ...
  def fromHash(hashType: HashType, hash: Array[Byte]): Either[MultiHashError, MultiHash] = {
    if (hash.length > 127) {
      Left(
        InvalidHashLength(
          "MultiHash length must not exceed 127 bytes, but you gave me " +
            hash.length
        )
      )
      ...
      Left(
        InvalidHashLength(
          "Hash type " + hashType.name + " has length of " +
            hashType.length + ", but you gave me " + hash.length
        )
      )
    } else {
      Right(MultiHash(hashType, hash))
    }
  }

  def fromBytes(multihash: Array[Byte]): Xor[MultiHashError, MultiHash] = {
    for {
      index    <- multihash.headOption.toRight(DecodingFailed("No multihash code present"))
      hashType <- lookup.get(index).toRight(UnknownMultiHashType("Unknown MultiHash type " + index))
  ...
  def fromHex(hex: String): Xor[MultiHashError, MultiHash] = {
    if (hex.length % 2 != 0) {
      Left(
        DecodingFailed(
          "A hex string must have an even number of digits, " +
